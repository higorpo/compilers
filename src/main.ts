...
let input: string | null = `def minhaFunc() { return; }`;
const hasFileIndicated = process.argv.some(value => value === '--f');
...
if (hasFileIndicated) {
    ...
    try {
        const data = fs.readFileSync(path.resolve('codes', fileName!), 'utf8');
        console.log('Arquivo carregado com sucesso!');
        input = data;
    } catch (err) {
        console.error('Não foi possível encontrar o arquivo indicado!');
        input = null;
    }
}
